...
Delete only `(reachability ∪ scanner) − executed − roots − cross_repo_allowlist`, then **iterate to a fixpoint**: removing one island orphans the next. With [sift](https://github.com/cjber/sift), `sift audit` runs the language's scanner and classifies each hit's fate: fully dead, test-only (remove the test too), or live via a non-source caller (e.g. an operations script or migration). `sift audit diff` scopes the audit to the diff. Use the project's configured roots, cross-repo allowlists, and framework exclusions (e.g. Vulture's `ignore_decorators`); group findings by file, largest-first. Classification supplies evidence for Phase 2, not permission to delete.
...
Dead/unreachable code · unused imports/variables/functions · unreferenced files · commented-out code · orphaned dependencies · deep nesting / needless complexity · **stale doc/spec/skill references to removed logic** · **duplicated / parallel implementations (consolidation candidates)**. With [sift](https://github.com/cjber/sift), use `sift audit` for the whole tree or `sift audit diff` for changed code. Other scanners include `vulture`, `ruff`/`flake8` (unused), `ts-prune`/`knip` (TS), `deadcode`, and `depcheck`. Reuse the project's tuned commands and documented false-positive list rather than re-deriving them. Every tool hit is a *candidate*, confirmed only after the Phase 2 reachability check.
