...
Delete only `(reachability ∪ scanner) − executed − roots − cross_repo_allowlist`, then **iterate to a fixpoint**: removing one island orphans the next. With [sift](https://github.com/agent-labs-dev/sift), `sift audit` runs the language's scanner and classifies each hit's fate: fully dead, test-only (remove the test too), or live via a non-source caller (e.g. an operations script or migration). `sift audit diff` scopes the audit to the diff. Use the project's configured roots, cross-repo allowlists, and framework exclusions (e.g. Vulture's `ignore_decorators`); group findings by file, largest-first. Classification supplies evidence for Phase 2, not permission to delete.
...
The five paths above have recurring concrete shapes where **an empty grep is the *expected* look of live code, not evidence of death.** These examples generalize across repos; consult the project's documented framework and persistence rules before deciding a candidate is dead.
...
- **Route substantial multi-file work through the repo's ship loop.** A sweep spanning many files/commits is implemented via the repo's canonical ship-and-review flow — one coherent PR per repo, branched off fresh main and kept up to date, reviewed as a unit — not a loose pile of edits. Runtime-test every changed user- or agent-visible surface with the project's verification commands before shipping.
- **Consolidation counter-rule: don't merge *coincidental* duplication.** Two blocks identical *today* that change for **different reasons** must stay separate — premature DRY couples them and the wrong abstraction costs more than the duplication. Merge only a genuine single source of truth (same concept, divergent copies); never fuse two concepts that merely look alike. Unsure whether duplication is essential or incidental → leave it (Rule of Three).
- **Retire public / cross-boundary surface by measurement, not static grep — never as a hot-table migration.** Removing an externally-reachable endpoint/field/column at scale is expand-contract + telemetry showing *zero live traffic over a window*, then deprecate, then delete — a static "no callers" is not decommission evidence (Phase 2). And never run the removal as bulk DDL/DML on a high-volume table (long lock = outage); do it as a bounded, partitioned backfill.
...
- **Wire the final check to the repo's real gate — but know its class.** Run the repo's mandatory pre-commit gate + full suite as a structural backstop. With [sift](https://github.com/agent-labs-dev/sift), the gate is the one in `.agents/skills/sift-project/SKILL.md`; `sift setup` writes that project skill with the gate and evidence commands. Follow the project's environment setup instructions before running them. These checks are a **static/structural backstop** — they may catch banned dynamic-access (`*attr`), mocked internals, migration drift, or unpinned durable-step names — but they do NOT replace the behavioral old-vs-new check this phase requires. Do both; a green gate on a deleted dynamically-reached symbol is exactly what a silent break looks like.
...
Dead/unreachable code · unused imports/variables/functions · unreferenced files · commented-out code · orphaned dependencies · deep nesting / needless complexity · **stale doc/spec/skill references to removed logic** · **duplicated / parallel implementations (consolidation candidates)**. With [sift](https://github.com/agent-labs-dev/sift), use `sift audit` for the whole tree or `sift audit diff` for changed code. Other scanners include `vulture`, `ruff`/`flake8` (unused), `ts-prune`/`knip` (TS), `deadcode`, and `depcheck`. Reuse the project's tuned commands and documented false-positive list rather than re-deriving them. Every tool hit is a *candidate*, confirmed only after the Phase 2 reachability check.
